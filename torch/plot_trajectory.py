...
def plot_trajectory(env_params: CartpoleEnvironmentParams, traj: Trajectory, filename_base: str, animate=False):
    ...
    for i in range(env_params.n):
        ...
        if env_params.s_max[0,i] < 10:
            ax[0,i].axhline(env_params.s_max[0,i], linestyle="--", color="tab:orange")
            ax[0,i].axhline(-env_params.s_max[0,i], linestyle="--", color="tab:orange")
        ax[0,i].set_xlabel(r"$t$")
        ax[0,i].set_ylabel(labels_s[i])
    for i in range(env_params.m):
        ax[1,i].plot(t[:-1], traj.u[:, i])
        ax[1,i].axhline(env_params.u_max, linestyle="--", color="tab:orange")
        ax[1,i].axhline(-env_params.u_max, linestyle="--", color="tab:orange")
