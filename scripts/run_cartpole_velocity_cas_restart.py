...
def main():
    ...
    plot_trajectory(solver_params=solver_params,
                    ...
                    traj=traj, filename_base="cartpole_velocity_cas_restart_1",
                    animate=False)

    # this is a nice example of how to use the already-built solver for
    # solves over shorter horizons. This will be handy for restarting along
    # trajectories (using a trajectory database).

    #n_restart = 0
    n_restart = traj.N // 2
    #n_restart = 42

    s0_restart = traj.s[n_restart].copy()
    #s0_restart = s0
    s_init = traj.s[n_restart:].copy()
    u_init = traj.u[n_restart:].copy()
    t0_restart = n_restart * traj.dt
    T_restart = scenario.T

    scenario_restart = TrajectoryScenario(s_goal=s_goal, s0=s0_restart, t0=t0_restart, T=T_restart)

    traj_restart = expert.trajectory(scenario_restart, s_init, u_init)

    plot_trajectory(solver_params=solver_params,
                    env_params=env_params,
                    traj=traj_restart, filename_base="cartpole_velocity_cas_restart_2",
                    animate=False)

    # pickle!
    #pickle_fest = [env_params, solver_params, traj]
    #with open("cartpole_velocity_cas_restart.pkl", "wb") as f:
